...
- **Cloud shadows**: Procedural cloud shadows in the daynight post-process shader. Uses 4-octave FBM (Fractal Brownian Motion) simplex noise to generate organic cloud shapes. World-space coordinates ensure shadows stay locked to the world, not the screen. Cloud offset animates over time (randomized speed 15-25 horizontal, 2-7 vertical world units/sec). Intensity is time-of-day modulated: peak at noon, zero at night/dawn/dusk. Tunable parameters: `cloud_scale` in game.c (smaller = bigger clouds, currently 0.005), smoothstep thresholds in daynight.fs (higher = fewer clouds). **WIP**: Current settings still produce small shadows; needs further tuning for 2-10 large clouds.
